  ...
  <script>
    ...
    function checkPwd() {
      ...
      if (upperCase.test(pwd)) {
        ...
        upperCaseSpan.innerHTML = "Password must contain one uppercase letter.";
        button.disabled = true;
      }
...
if (isset($_POST['SAVE'])) {
  ...
  $stmt = $conn->prepare("INSERT INTO reg(email, password) VALUES (?, ?)");
  $stmt->bind_param("ss", $e, $p);

  if ($stmt->execute()) {
    header("Location: index.php");  // corrected header syntax
    exit();
  } else {
